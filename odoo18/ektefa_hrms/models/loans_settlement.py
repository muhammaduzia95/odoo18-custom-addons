...
class EktefaLoansSettlement(models.Model):
    _name = 'ektefa.loans.settlement'
    _description = 'Ektefa Loans Settlement'
    _order = 'id DESC'
    ...
    ektefa_loan_settlement_id = fields.Char(string="Ektefa Loan Settlement ID")
    ektefa_loan_settlement_amount = fields.Float(string="Loan Settlement Amount")
    ...
    ektefa_settlement_method_id = fields.Char(string="Settlement Method ID")
    ektefa_settlement_method_name_en = fields.Char(string="Name EN")
    ektefa_settlement_method_name_ar = fields.Char(string="Name AR")

    ektefa_settlement_date = fields.Date(string="Settlement Date", compute="_compute_settlement_date", store=True,)
    ektefa_settlement_date_text = fields.Char(string="Settlement Date Text")
    ...
    @api.depends('ektefa_settlement_date_text')
    def _compute_settlement_date(self):
        for loan in self:
            try:
                settlement_date = datetime.strptime(loan.ektefa_settlement_date_text, "%Y-%m-%d").date()
                loan.write({'ektefa_settlement_date':  settlement_date})
            except Exception as e:
                loan.ektefa_settlement_date = False
    ...
    def action_confirm(self):
        ...
        for slip in self:
            ...
            credit_account_id = self.env.company.ektefa_loans_settlement_credit_account
            debit_account_id = self.env.company.ektefa_loans_settlement_debit_account

            name = ""
            if slip.ektefa_settlement_method_name_en or slip.ektefa_settlement_method_name_ar:
                name = slip.ektefa_settlement_method_name_en or slip.ektefa_settlement_method_name_ar
            line_name = f"Settlement Method: {name}, Comments: {self.ektefa_comments}"
            if credit_account_id and debit_account_id:
                move_lines.append((0, 0, {
                    'account_id': credit_account_id.id,
                    'name': line_name,
                    'debit': 0,
                    'credit': abs(slip.ektefa_loan_settlement_amount),
                    'partner_id': self.partner_id.id if self.partner_id else False,
                }))
                move_lines.append((0, 0, {
                    'account_id': debit_account_id.id,
                    'name': line_name,
                    'debit': abs(slip.ektefa_loan_settlement_amount),
                    'credit': 0,
                    'partner_id': self.partner_id.id if self.partner_id else False,
                }))
            ...
            move_vals = {
                ...
                'date': slip.ektefa_settlement_date,
                'ref': f"{slip.name} - ({slip.ektefa_settlement_date})",
                'line_ids': move_lines,
                'move_type': 'entry',
    ...
    def create(self, list_vals):
        ...
        return super(EktefaLoansSettlement, self).create(list_vals)

    def write(self, vals):
        if not vals.get('company_id'):
            vals['company_id'] = self.env.company.id
        return super(EktefaLoansSettlement, self).write(vals)
