...
class TtiSiUom(models.Model):
    _name = 'tti.si.uom'
    _description = 'Tti Sample Information UOM'
    _order = "id desc, name"
    _check_company_auto = True
    _check_company_domain = models.check_company_domain_parent_of

    name = fields.Char(string='UOM Name', required=True)
    code = fields.Char(string='UOM Code', copy=False , readonly=False)
    ...
    def create(self, list_vals):
        ...
        return super(TtiSiUom, self).create(list_vals)

    def write(self, vals):
        if not vals.get('company_id'):
            vals['company_id'] = self.env.company.id
        return super(TtiSiUom, self).write(vals)
