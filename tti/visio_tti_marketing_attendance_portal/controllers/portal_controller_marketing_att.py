# D:\Visiomate\Odoo\odoo18\custom_addons\tti\visio_tti_marketing_attendance_portal\controllers\portal_controller.py
...
class PortalMarketingAttendance(http.Controller):
    ...
    def portal_marketing_attendance_list(self, **kw):
        ...
        Requests = request.env["marketing.attendance.request"].sudo()
        req_domain = [("employee_id", "=", employee.id)] if employee else [("id", "=", 0)]
        marketing_requests = Requests.search(req_domain, order="date desc")
        ...
        return request.render("visio_tti_marketing_attendance_portal.marketing_attendance_list", values)
    ...
    def portal_marketing_attendance_create(self, **kw):
        ...
        return request.render("visio_tti_marketing_attendance_portal.marketing_attendance_create", values)
    ...
    def portal_marketing_attendance_submit(self, **post):
        ...
        employee = self._get_employee()
        if not employee:
            return request.render("visio_tti_marketing_attendance_portal.marketing_portal_error", {
        ...
        request.env["marketing.attendance.request"].sudo().create({
            "employee_id": employee.id,
